...
void ui_item_posicion::actualizar()
{
    if(caso)
    {
        ui->pushButton_addProducto->setText("Crear");
    }
    else
        ui->pushButton_addProducto->setText("Guardar");
...
void ui_item_posicion::actualizar_producto()
{
    ...
    QString idpro=query1.value(0).toString();
    cout<<"Id item"<<idItem.toStdString()<<"ID PRODUCTO"<<idpro.toStdString()<<endl;
    QSqlQuery query;
...
void ui_item_posicion::on_pushButton_guardar_clicked()
{
    ...
    qDebug()<<"iditem_posicion : "<<idItem<<endl;
